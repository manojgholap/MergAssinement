SampleApplicationModule
    .controller('newsController', function($rootScope, $scope, $location, $http, store, $timeout, $routeParams , Upload) {
        ...
        $scope.allNews = function() {
            $http.get(baseURL + 'getPortalNews').success(function(res) {
                $scope.news = res;
        ...
        $scope.itemdata = function(id) {
            $location.path('/view_news/' + id);
        };

        var id = $routeParams.id; 
        if (id) {

            $http.get(baseURL + 'getNewsByID/' + id).success(function(res) {
            	console.log(res);
                 $scope.newsObject = res[0];
            }).error(function(error) {
